...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 23:22:10 2021
...
def d_ecg_peaks(d_ecg, peaks_d_ecg, time, heightper, distanceper):
    ...
    Output: Rwave peaks"""
def Rwave_peaks(ecg, d_ecg, Rwave_peaks_d_ecg, time, trial):   
    ...
    plt.figure()  
    plt.xlabel('Time [s]') 
    plt.title('R-wave peaks step of ' + trial)
    plt.plot(time, ecg, color = 'b', label = 'ECG')
    plt.plot(time[Rwave], ecg[Rwave], "x", color = 'r')
    plt.ylabel('ECG')
    return Rwave_t



def newX_axis(distance):
    X_list =[0]*len(distance)
    X_val  = 0 
    for i in range(len(distance)):
        X_val = X_val + distance[i]
        X_list[i] = X_val
    return X_list